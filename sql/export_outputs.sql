...
.once data/sql_latest_asset_ranking.csv
WITH latest_date AS (
    SELECT MAX(observation_date) AS observation_date FROM asset_returns
),
recent_prices AS (
    SELECT
        r.asset_id,
        ...
        a.risk_band,
        r.close_price,
        (r.close_price / r.rolling_20_observation_price) - 1
            AS price_vs_rolling_average
...
SELECT
    ...
    ROUND(price_vs_rolling_average * 100, 2)
        AS price_vs_rolling_average_pct,
    DENSE_RANK() OVER (ORDER BY price_vs_rolling_average DESC)
        AS market_price_rank
FROM recent_prices
ORDER BY market_price_rank;

.once data/sql_portfolio_monthly.csv
SELECT
...
.once data/sql_asset_monthly.csv
...
SELECT
    ...
    ROUND(average_weight * return_sum * 100, 2)
        AS contribution_to_return_pct,
    ROUND(
